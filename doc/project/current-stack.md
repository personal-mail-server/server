...
현재 로그인 이후에는 테스트용 메일 주소 목록, 상세, 생성, 후보 생성 흐름을 같은 정적 페이지 안에서 검증할 수 있다. 수정/삭제 UI는 아직 포함되지 않는다.
...
추가로 현재는 테스트용 메일 주소 관리 기능의 초기 단계로, DB 마이그레이션과 내부 저장 모델/리포지토리 패키지, 후보 생성 API, 생성 API, 목록/상세 조회 API, 수정 API, 논리 삭제 API가 반영되어 있다. 프론트엔드는 목록/상세/생성/후보 생성 UI까지 반영되어 있고, 수정/삭제 UI는 아직 포함되지 않는다.
